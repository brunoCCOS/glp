\tikzset{
    ...
    \fbox{
        \begin{minipage}{0.97\linewidth}
			...
			\textit{Scalars and antipode ($\text{-}1$). The antipode is
			self-dual: its white variant is defined equal to the black one,
			so it is unaffected by the colour-swap duality.}
			\begin{center}\begin{tabular}{r c c c}
                \diagrow{(s.add)}
                    {
                      \begin{scope}[shift={(0.000,0.500)}]
