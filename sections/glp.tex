...
\begin{itemize}
    ...
    \[
    (x,y) \mapsto \inf_z \{ c^\top z \mid Az \ge B[x\ y]^\top + b \},
    \]
...
From this simple axioms, we get immediatly another core property of linear programs,
namely, that they are invariant to stochastics matrices.
...
\begin{remark}[Invariance under column-stochastic matrices]\label{rmk:stochastic}
    ...
    \begin{figure}[H]
        \centering
        \input{paper/tikz/RemarkStochastic.tex}
    \end{figure}
\end{remark}

Such result is particularly interesting because it follows a pattern present in
every algebra presented so far. In the digrammatic calculi of Linear subspaces
(GLA) we can demonstrate that surjective matrices are very special because the
$\inlinegen{codiscard}$ is invariant to it, which is a very important feature
in the completeness of the language itself. The same occurs in GPA, where non-negative
matrices slide through the $\inlinegen{ge}$. Here, the new class of
matrices that interact with the new generator $\inlinegen{linear}$ is that 
of stochastic matrices. Other works have spotted the same pattern, such as 
\cite{stein2024gqa} which shows that a generator representing the $l_2$ norm is 
invariant to orthogonal matrices (also are normal distributed random variables).

Is not yer entirely clear the relationship between each class of morphisms and
that class of matrices and how they emerge. However is a
signature and very interesting property of such languages.

With the SMT above we now define the complete category.
...
\begin{theorem}[Normal Form]\label{thm:NormalFormGLP}
    ...
    \begin{figure}[H]
        \centering
        \input{paper/tikz/NormalFormGLP.tex}
    \end{figure}
    With $A$ and $b$ being an affine transformation. Namely:
    \begin{figure}[H]
        \centering
        \input{paper/tikz/AffineTransformation.tex}
    \end{figure}
\end{theorem}
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \begin{figure}[H]
            \centering
            \input{paper/tikz/ProofNormalFormBaseLinear.tex}
        \end{figure}
        \item A diagram $c \in \mathbf{GPA}$

        \begin{figure}[H]
            \centering
            \input{paper/tikz/ProofNormalFormBaseGPA.tex}
        \end{figure}
    \end{itemize}
    ...
    \begin{figure}[H]
        \centering
        \input{paper/tikz/ProofNormalFormVerticalComp.tex}
    \end{figure}
    ...
    \begin{figure}[H]
        \centering
        \input{paper/tikz/ProofNormalFormHorizontalComp.tex}
    \end{figure}
\end{proof}
...
\begin{corollary}[Epigraph Form]\label{cor:epigraph form}
    ...
    \begin{figure}[H]
        \centering
        \input{paper/tikz/EpigraphFormGLP.tex}
    \end{figure}
...
\begin{proof}
    ...
    \input{paper/tikz/EpigraphProof.tex}
...
\begin{proof} 
    Any RHS Parametric linear program can be written as 
    $\inf_z \{ c^\top z \mid Ax + b \ge By$ therefore is enough to show that we
    can always depict such a problem. The following diagram provides the required
    syntactic representative:
    \begin{figure}[H]
        \centering
        \input{paper/tikz/ProofExpressivity.tex}
    \end{figure}
\end{proof}
...
\begin{proof}
    ...
    \begin{figure}[H]
        \centering
        \input{paper/tikz/ProofSoundness.tex}
    \end{figure}
\end{proof}
...
\begin{proof}
    ...
    \begin{figure}[H]
        \centering
        \input{paper/tikz/ProofCompletenessLemma.tex}
    \end{figure}
...
This result does more than unlock a new algebraic and computational way to
interact with LP problems: it relocates linear programming itself. A linear
program is ordinarily presented as a function to be evaluated, an
$x \mapsto c^\top x$ subject to constraints, solved from the outside by an
algorithm. Here it is instead a morphism, an object with an interface that
composes with others of its kind, and solving it, evaluating it, and
eliminating one of its variables are all the same operation, composition in
$\mathbf{LPRel}$. Optimization stops being a technique applied to a
relational structure and becomes a property internal to it, and linear
programming, in particular, is lifted out of the setting of functional
thinking and given a new home as a relational field embedded in the
Extended-Lawvere Quantale category.

The \textbf{GLP} theory, in turn, has a natural extension to an even more expressive theory
when enriched with the gray arrow generator from \textbf{GQA} \cite{stein2024gqa}.
We believe that the class of quadratic programs could be, just as was done in this work,
fully axiomatized by such broader SMIT. However, such studies are left for future works,
along with other possible applications of current results.
