...
On set-theoretical foundations, relations are among the most primitive notions
available, and yet they already support remarkably powerful abstractions: many
other structures, functions among them, are derived from relations alone.
Later we shall see that relations sit comfortably within the language of
category theory, which in turn offers a natural vocabulary for describing them.

We begin with the most basic kind of relation:

\begin{definition}[Binary Relations]\label{def:binary-relations}
...
Each of these three axioms earns its place. The complete lattice gives
meaning to the join $\bigvee$, the operation that eliminated the
intermediate variable $y$ in the transitivity example of the introduction.
The monoid $(Q,\otimes,e)$ plays the role of combining evidence along a
chain of relations, with $e$ the trivial combination that changes nothing.
Distributivity is what lets us freely interchange this join and this
product, precisely the property that allows composition and variable
elimination to be reduced to the same operation, rather than two operations
that merely happen to look alike.
...
\begin{example}
	...
	This also settles the indicator convention adopted in
	Definition~\ref{def:binary-relations}, where relations were valued in
	$\{0,+\infty\}$ rather than $\{0,1\}$. The two are the same quantale: the map
	$1 \mapsto 0$, $0 \mapsto +\infty$ sends $\lor$ to $\min$, $\land$ to $+$,
	and the unit $1$ to the unit $0$, hence is an isomorphism of the truth
	quantale onto the sub-quantale $\{0,+\infty\}$ of the Lawvere quantale below.
	We work with $\{0,+\infty\}$ throughout, so that Boolean relations sit inside
	the cost-valued ones without any change of convention.
	...
	\begin{itemize}
		...
		\item The monoidal product is addition:
		      \[
			      a \otimes b := a + b,
		      \]
		      where addition is extended to $\overline{\mathbb{R}}$ by
		      $a + (+\infty) = +\infty$ for every $a$, and
		      $a + (-\infty) = -\infty$ for every $a \neq +\infty$.
		      In particular we adopt the convention
		      \[
			      (+\infty) + (-\infty) := +\infty,
		      \]
		      so that $+\infty$ is absorbing. This is Moreau's
		      \emph{inf-addition} \cite{rockafellar1970convex}, and it is the
		      only extension under which the structure below is a quantale.
...
The convention above is not a matter of taste. Without it, $+$ is only
partially defined on $\overline{\mathbb{R}}$, so
$(\overline{\mathbb{R}},+,0)$ fails to be a monoid and the quantale axioms
cannot even be stated. The next proposition checks that inf-addition repairs
this.

\begin{proposition}
	$(\overline{\mathbb{R}},\geq,+,0)$ with inf-addition is a commutative
	quantale.
\end{proposition}

\begin{proof}
	Since $\overline{\mathbb{R}}$ under the reversed order is a complete lattice
	with $\bigvee A = \inf A$, $\bot = \bigvee \varnothing = +\infty$ and
	$\top = -\infty$, only the monoid structure and distributivity need checking.

	\medskip
	\noindent
	\textbf{(1) Monoid.}
	Inf-addition is commutative by construction and total, since the one
	ambiguous sum is now assigned the value $+\infty$. For associativity, note
	that $a + b = +\infty$ exactly when $a = +\infty$ or $b = +\infty$, and
	$a + b = -\infty$ exactly when neither is $+\infty$ and at least one is
	$-\infty$. Both conditions are symmetric in the summands and depend only on
	the multiset $\{a,b\}$, so $(a+b)+c$ and $a+(b+c)$ agree on every triple:
	each equals $+\infty$ if some argument is $+\infty$, else $-\infty$ if some
	argument is $-\infty$, else the ordinary finite sum. The unit is $0$.

	\medskip
	\noindent
	\textbf{(2) Distributivity.}
	Let $a \in \overline{\mathbb{R}}$ and $\{b_i\}_{i \in I}$ be any family. We
	must show $a + \inf_i b_i = \inf_i (a + b_i)$, where both sides are $+\infty$
	when $I = \varnothing$.

	If $a = +\infty$, both sides are $+\infty$ by absorption. If $a$ is finite,
	the identity is the standard translation invariance of the infimum. If
	$a = -\infty$ and every $b_i = +\infty$, then $\inf_i b_i = +\infty$ and both
	sides are $+\infty$; if instead some $b_{i_0} < +\infty$, then
	$\inf_i b_i < +\infty$, so the left side is $-\infty$, and the right side is
	$-\infty$ as well because $a + b_{i_0} = -\infty$.
	Commutativity gives distributivity in the other variable.
\end{proof}
...
\begin{remark}
	...
	\begin{figure}[H]
		\centering
		\input{paper/tikz/FrobeniusGeneratorLegend.tex}
	...
	\begin{figure}[H]
		\centering
		\input{paper/tikz/FrobeniusLaw.tex}
	\end{figure}
\end{remark}
...
\begin{definition}\label{def:arrows}
	...
	\begin{figure}[H]
		\centering
		\input{paper/tikz/RightAlgebra.tex}
	\end{figure}
	without their symmetric
	\begin{figure}[H]
		\centering
		\input{paper/tikz/LeftAlgebra.tex}
	\end{figure}
