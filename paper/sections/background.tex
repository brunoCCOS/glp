...
Relations encode the notion of matrices and matrix multiplication. A $Q$-valued relation $R : X \times Y \to Q$ is nothing but a \emph{$Q$-valued matrix} indexed by $X \times Y$, and composition of $Q$-valued relations corresponds precisely to matrix multiplication in $Q$, where ordinary addition is replaced by the join $\bigvee$ and ordinary multiplication by the monoidal product $\otimes$. Worth noticing, however, that in this case, matrices can indeed be infinite (countable), different from the usual finite case.
...
Graphical Polyhedral Algebra \cite{bonchi2021polyhedral, bonchi2021.9farkas}, the established theory on which our contribution builds, geometrically goes beyond affine spaces, representing polyhedra as linear-inequality constraints. We begin by recalling its standard definitions.
...
\begin{definition}[$\mathbf{GPA}$ normal form]\label{def:gpanormalform}
	...
	written as a relation--\inlinegen{ge}--comatrix triple
	\begin{figure}[H]
		\centering
		\input{tikz/NormalFormGPA.tex}
	\end{figure}
...
Applying the polar operator (the polyhedral analogue of Farkas duality, exchanging the $H$- and $V$-representations \cite{bonchi2021.9farkas}) yields a second, conic-hull normal form. These drive the presentation theorems below, whose proofs we recall from \cite{bonchi2021polyhedral}.
