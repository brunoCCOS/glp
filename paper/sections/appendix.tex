...
\begin{remark}[Invariance under column-stochastic matrices]\label{rmk:stochastic}
	...
	the comatrix is opened into its definition (one comultiplication per input,
	one coscalar per entry, one cocopy per output); \textit{linearity} merges all
	\inlinegen{linear} generators into a single one feeding a comultiplication
	tree; \textit{coassoc} and \textit{cocomm} regroup the branches so that the
	entries of each column become adjacent; the mirrored \textit{s.add} axiom then
	sums the coefficients of each column, which equal $1$ by assumption, leaving
	bare wires; a final application of \textit{linearity} splits the diagram back
	into one \inlinegen{linear} generator per output. We display the proof for a
	...
	\begin{figure}[H]
		\centering
		\input{tikz/RemarkStochastic.tex}
	\end{figure}
\end{remark}
