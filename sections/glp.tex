...
\section{Semantics: the props \texorpdfstring{$\mathbf{LPRel}$}{LPRel}
and \texorpdfstring{$\mathbf{PolyFun}$}{PolyFun}}
...
section, and they are what allow us to present the semantic categories
of this chapter in two equivalent ways.
...
With the equivalences above in place, we now define the two semantic
categories of this chapter. The first, $\mathbf{LPRel}$, describes
morphisms as parametric linear programs; the second, $\mathbf{PolyFun}$,
describes them as polyhedral functions. The equivalence between the two
follows immediately from the propositions above, and we will use both
descriptions interchangeably according to which is more convenient.
