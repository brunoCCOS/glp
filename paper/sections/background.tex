...
(the choice of $\infty$ will be justified later; normal choices here are actually $\{\text{true},\text{false}\}$ or $\{0,1\}$, the latter being isomorphic to our choice).
...
The key mechanism enabling this is the infimal composition present in the definition of the Cost category: composing two relations automatically performs variable elimination, which is precisely the operation underlying optimization.
...
Building on the above definition, we stablish the following notation to easy the
...
Our contribution is built on top of Graphical Polyhedral Algebra \cite{bonchi2021polyhedral, bonchi2021.9farkas}. Geometrically it goes beyond affine spaces, representing polyhedra as linear-inequality constraints. We begin by recalling the standard definitions.
